...
const WalkingScreen = ({navigation}) => {
  ...
  // we're going to hard code the text box with some text but in a real implementation of the app this would be where the route specific infomation is subbed in
  const [checkpoint, setCheckpoint] = useState(1);
  const [totalCheckpoints, setTotalCheckpoints] = useState(40);
  const [connectivity, setConnectivity] = useState('good');
  const [pace, setPace] = useState(4.5);
  const [funFact, setFunFact] = useState('Fun facts here');
  ...
  return (
    ...
    <View style={styles.infoContainer}>
      <Text style={styles.infoText}>
        Checkpoint {checkpoint} of {totalCheckpoints}
      </Text>
      <Text style={styles.infoText}>Connectivity: {connectivity}</Text>
      <Text style={styles.infoText}>Pace: {pace} kph</Text>
  ...
  const styles = StyleSheet.create({
    ...
    photoContainer: {
      flex: 3, // Adjust this value to control the height of the photo container
      width: Dimensions.get('window').width * 0.95,
      overflow: 'hidden',
    },
    photo: {
